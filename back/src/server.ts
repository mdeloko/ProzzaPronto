...
ioServer.on("connection",(client)=>{    
    ...
    client.on("requestRoom",(sender:string,receiver:string,actRoom:string,callback)=>{
        if(sender&&receiver){
    ...
    client.on("message", async(userSent:string,userReceive:string,msg:string)=>{
        try{
    ...
    client.on("retrieveUserId",async (username:string,callback)=>{
        const usr = await uc.findUserByUsername(username);
        callback(usr?.id)
    });
    client.on("retrieveUsernameIdByEmail",async (email:string,callback)=>{
    ...
    client.on("retrieveFriendList",async(username:string,callback)=>{
